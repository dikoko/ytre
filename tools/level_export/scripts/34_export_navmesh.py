...
def export_map(code: str, out_dir: Path = OUTPUT_DIR, dry_run: bool = False) -> dict:
    ...
    result = {"navmesh": 0, "walls": 0, "cells": 0}
    ...
    if has_navmesh or grid is not None:
        if not dry_run:
...
def _included_map_codes() -> list[str]:
    """Same inclusion rule as the level catalog: a real Map.IRD/{code}/
    folder with a `.qqq` file present."""
    codes = []
...
def main() -> None:
    ...
    summary = (f"navmeshes: {navmesh_count} ({navmesh_cells_total} cells total), "
               f"walls: {walls_count}, no-navmesh: {no_navmesh_count}")
    if error_count:
