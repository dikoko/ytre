...
func _ff_material(src: StandardMaterial3D, sun_direction: Vector3,
	...
	# ADAPTATION (ytavatar port): ytlevel calls
	# PointLights.apply(self, mat) here to bind a map's .plt point-light
	# array — that class and the per-map point-light data it reads do not
	# exist in this repo (the avatar tool has no map/.plt concept, only the
	# portrait-rig sun). Effect materials fall back to prop_ff.gdshader's
	# uniform defaults (point_light_count = 0), which is exactly correct
	# here: skill effects never sit in a lit map, only beside a character.
	_converted[key] = mat
	return mat
