extends Node3D
## Test scene for verifying avatar parts attach to skeleton correctly.
##
## Instructions:
## 1. Open test_avatar_parts.tscn in Godot editor
## 2. Add male_base_materials.glb as child node (drag from FileSystem)
## 3. Run the scene
##
## Controls:
...
# Node references - these will be found dynamically when male_base is added
...
const PARTS_PATH := "res://assets/avatars/parts/male/"
const METADATA_PATH := "res://assets/avatars/parts/parts_metadata.json"
...
const MATERIAL_TO_TEXTURE: Dictionary = {
	"upper": "male_upper_M0000.tga",  # Default outfit top
	"arm": "male_arm_M0000.tga",      # Arm skin
	"foot": "male_foot_M0000.tga",    # Foot skin
	"hand": "male_hand_M0000.tga",    # Hand skin
	"leg": "male_leg_M0000.tga",      # Leg skin
	"lower": "male_lower_M0000.tga",  # Default outfit bottom
	"hair_scalp": "male_hair_M0000.tga",  # Scalp - uses same hair texture as strands (both hidden when hair part equipped)
	"hair_strands": "male_hair_M0000.tga",  # Hair strands (hidden when hair part equipped)
...
# Face types available (10 types for testing)
...
func _get_face_texture_path(face_type: String, blink_frame: int) -> String:
	...
	var texture_name = "male_face_M" + face_type + str(blink_frame) + ".tga"
...
"""
	...
	if not base_avatar:
		push_warning("No base avatar found. Add male_base_materials.glb as child node in editor.")
		return
...
func _unequip() -> void:
	...
	if animation_mode == AnimMode.EQUIP:
		...
		_update_available_animations()
		var stand_idx = animation_list.find("basic_stand")
